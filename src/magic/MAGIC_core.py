...
def magic(
    data, n_pca_components=20, random_pca=True, t=6, k=30, ka=10, epsilon=1, rescale=99
):
    ...
    L = compute_markov(
        pca_projected_data, k=k, epsilon=epsilon, distance_metric="euclidean", ka=ka
    )
...
def impute_fast(data, L, t, rescale_percent=0, L_t=None, tprev=None):
    ...
    if rescale_percent != 0:
        if len(np.where(data_new < 0)[0]) > 0:
            print(
                "Rescaling should not be performed on log-transformed "
