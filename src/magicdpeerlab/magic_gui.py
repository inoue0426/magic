...
class magic_gui(tk.Tk):
    ...
    def initialize(self):
        ...
        self.fileMenu.add_command(
            label="Load saved session from pickle file", command=self.loadPickle
        )
        self.fileMenu.add_command(
            label="Save selected data to csv",
            state="disabled",
            command=self.saveDataToCSV,
        )
        self.fileMenu.add_command(
            label="Save session to pickle file", state="disabled", command=self.saveData
        )
        ...
        self.analysisMenu.add_command(
            label="MAGIC", state="disabled", command=self.runMagic
        )
        ...
        self.visMenu.add_command(
            label="Scatter plot", state="disabled", command=self.scatterPlot
        )
        self.visMenu.add_command(
            label="plot % explained pca", state="disabled", command=self.plotPCAVariance
        )
        ...
        tk.Label(
            self,
            text=u"Markov Affinity-based Graph Imputation of Cells",
            font=("Helvetica", 25),
            fg="black",
            bg="white",
            padx=100,
    ...
    def loadCSV(self):
        self.dataFileName = filedialog.askopenfilename(
            title="Load data file", initialdir="~/.magic/data"
        )
        if self.dataFileName != "":
            ...
            tk.Label(self.fileInfo, text=u"Rows:", fg="black", bg="white").grid(
                column=0, row=3
            )
            self.rowVar = tk.IntVar()
            self.rowVar.set(0)
            tk.Radiobutton(
                self.fileInfo, text="Cells", variable=self.rowVar, value=0
            ).grid(column=1, row=3)
            tk.Radiobutton(
                self.fileInfo, text="Genes", variable=self.rowVar, value=1
            ).grid(column=2, row=3)

            tk.Label(
                self.fileInfo,
                text=u"Number of additional rows/columns to skip after gene/cell names",
            ...
            tk.Button(
                self.fileInfo,
                text="Compute data statistics",
                command=partial(self.showRawDataDistributions, file_type="csv"),
            ).grid(column=1, row=7)
            ...
            tk.Label(
                self.fileInfo,
                text=u"Filter by molecules per cell. Min:",
                fg="black",
                bg="white",
            ).grid(column=0, row=8)
            tk.Entry(self.fileInfo, textvariable=self.filterCellMinVar).grid(
                column=1, row=8
            )

            self.filterCellMaxVar = tk.StringVar()
            tk.Label(self.fileInfo, text=u" Max:", fg="black", bg="white").grid(
                column=2, row=8
            )
            tk.Entry(self.fileInfo, textvariable=self.filterCellMaxVar).grid(
                column=3, row=8
            )
            ...
            tk.Label(
                self.fileInfo,
                text=u"Filter by nonzero cells per gene. Min:",
                fg="black",
                bg="white",
            ).grid(column=0, row=9)
            tk.Entry(self.fileInfo, textvariable=self.filterGeneNonzeroVar).grid(
                column=1, row=9
            )
            ...
            tk.Label(
                self.fileInfo,
                text=u"Filter by molecules per gene. Min:",
                fg="black",
                bg="white",
            ).grid(column=0, row=10)
            tk.Entry(self.fileInfo, textvariable=self.filterGeneMolsVar).grid(
                column=1, row=10
            )
            ...
            tk.Checkbutton(
                self.fileInfo,
                text=u"Normalize by library size",
                variable=self.normalizeVar,
            ).grid(column=0, row=11, columnspan=4)

            # log transform
            self.logTransform = tk.BooleanVar()
            self.logTransform.set(False)
            tk.Checkbutton(
                self.fileInfo, text=u"Log-transform data", variable=self.logTransform
            ...
            tk.Label(
                self.fileInfo,
                text=u"Pseudocount (for log-transform)",
                fg="black",
                bg="white",
            ).grid(column=1, row=12)
            tk.Entry(self.fileInfo, textvariable=self.pseudocount).grid(
                column=2, row=12
            )
            ...
            tk.Button(
                self.fileInfo,
                text="Load",
                command=partial(self.processData, file_type="csv"),
    ...
    def loadMTX(self):
        ...
        if self.dataFileName != "":
            ...
            tk.Button(
                self.fileInfo,
                text="Select gene names file",
                command=self.getGeneNameFile,
            ).grid(column=0, row=2)

            tk.Button(
                self.fileInfo,
                text="Compute data statistics",
                command=partial(self.showRawDataDistributions, file_type="mtx"),
            ).grid(column=0, row=3)
            ...
            tk.Checkbutton(
                ...
                text=u"Normalize by library size",
                variable=self.normalizeVar,
            ).grid(column=0, row=7, columnspan=4)
            ...
            tk.Button(
                self.fileInfo,
                text="Load",
                command=partial(self.processData, file_type="mtx"),
            ).grid(column=2, row=10)

            self.wait_window(self.fileInfo)

    def load10x(self):
        self.dataDir = filedialog.askdirectory(
            title="Select data directory", initialdir="~/.magic/data"
        )
        if self.dataDir != None:
            ...
            self.geneVar.set(0)
            tk.Radiobutton(
                self.fileInfo, text="Use ensemble IDs", variable=self.geneVar, value=1
            ).grid(column=1, row=2)
            tk.Radiobutton(
                self.fileInfo, text="Use gene names", variable=self.geneVar, value=0
            ).grid(column=2, row=2)

            tk.Button(
                self.fileInfo,
                text="Compute data statistics",
                command=partial(self.showRawDataDistributions, file_type="10x"),
            ).grid(column=0, row=3)
            ...
            tk.Button(
                self.fileInfo,
                text="Load",
                command=partial(self.processData, file_type="10x"),
            ).grid(column=2, row=10)
    ...
    def load10xHDF5(self):
        ...
        if self.dataFileName != None:
            ...
            tk.Label(self.fileInfo, text=u"Genome:", fg="black", bg="white").grid(
                column=0, row=2
            )
            ...
            tk.Checkbutton(
                ...
                text=u"Normalize by library size",
                variable=self.normalizeVar,
            ).grid(column=0, row=8, columnspan=4)
            ...
            tk.Button(
                self.fileInfo,
                text="Load",
                command=partial(self.processData, file_type="10x_HDF5"),
            ).grid(column=2, row=11)

            self.wait_window(self.fileInfo)

    def getGeneNameFile(self):
        self.geneNameFile = filedialog.askopenfilename(
            title="Select gene name file", initialdir="~/.magic/data"
        )
        tk.Label(
            self.fileInfo, text=self.geneNameFile.split("/")[-1], fg="black", bg="white"
        ).grid(column=1, row=2)

    def loadPickle(self):
        self.dataFileName = filedialog.askopenfilename(
            title="Load saved session", initialdir="~/.magic/data"
        )
        if self.dataFileName != "":
            # pop up data options menu
            self.fileInfo = tk.Toplevel()
            ...
            tk.Button(
                self.fileInfo,
                text="Load",
                command=partial(self.processData, file_type="pickle"),
            ).grid(column=2, row=2)
    ...
    def processData(self, file_type="csv"):
        ...
        if file_type == "csv":  # sc-seq data
            ...
            scdata = magic.mg.SCData.from_mtx(
                os.path.expanduser(self.dataFileName),
                os.path.expanduser(self.geneNameFile),
                normalize=False,
            )
        elif file_type == "10x":
            scdata = magic.mg.SCData.from_10x(
                self.dataDir, use_ensemble_id=self.geneVar.get(), normalize=False
            )
        elif file_type == "10x_HDF5":
            scdata = magic.mg.SCData.from_10x_HDF5(
                os.path.expanduser(self.dataFileName),
                self.genomeVar.get(),
                use_ensemble_id=self.geneVar.get(),
                normalize=False,
            )

        if file_type != "pickle":
            if (
                len(self.filterCellMinVar.get()) > 0
                or len(self.filterCellMaxVar.get()) > 0
                or len(self.filterGeneNonzeroVar.get()) > 0
                or len(self.filterGeneMolsVar.get()) > 0
            ):
                scdata.filter_scseq_data(
                    filter_cell_min=int(self.filterCellMinVar.get())
                    if len(self.filterCellMinVar.get()) > 0
                    else 0,
                    filter_cell_max=int(self.filterCellMaxVar.get())
                    if len(self.filterCellMaxVar.get()) > 0
                    else np.inf,
                    filter_gene_nonzero=int(self.filterGeneNonzeroVar.get())
                    if len(self.filterGeneNonzeroVar.get()) > 0
                    else 0,
                    filter_gene_mols=int(self.filterGeneMolsVar.get())
                    if len(self.filterGeneMolsVar.get()) > 0
                    else 0,
                )
        ...
        self.data[self.fileNameEntryVar.get()] = {
            "scdata": scdata,
            "state": tk.BooleanVar(),
        ...
        self.data_list.insert(
            "",
            "end",
            text=self.fileNameEntryVar.get()
            + " ("
            + str(scdata.data.shape[0])
            + " x "
            + str(scdata.data.shape[1])
            + ")",
            open=True,
        )
        ...
        self.concatButton = tk.Button(
            self,
            text=u"Concatenate selected datasets",
            state="disabled",
            wraplength=80,
            command=self.concatenateData,
        )
    ...
    def saveData(self):
        for key in self.data_list.selection():
            ...
            pickleFileName = filedialog.asksaveasfilename(
                title=name + ": save data", defaultextension=".p", initialfile=name
            )
    ...
    def concatenateData(self):
        ...
        tk.Label(
            self.concatOptions, text=u"New data set name:", fg="black", bg="white"
        ).grid(column=0, row=0)
        ...
        tk.Radiobutton(
            self.concatOptions, text="Concatenate rows", variable=self.colVar, value=1
        ).grid(column=1, row=1)

        self.joinVar = tk.BooleanVar()
        self.joinVar.set(True)
        tk.Checkbutton(
            self.concatOptions, text=u"Outer join", variable=self.joinVar
        ).grid(column=0, row=2, columnspan=2)

        tk.Button(
            self.concatOptions, text="Concatenate", command=self._concatenateData
        ).grid(column=1, row=3)
        tk.Button(
            self.concatOptions, text="Cancel", command=self.concatOptions.destroy
        ).grid(column=0, row=3)
    ...
    def _concatenateData(self):
        ...
        for key in self.data_list.selection():
            to_concat.append(
                self.data[self.data_list.item(key)["text"].split(" (")[0]]["scdata"]
            )

        scdata = to_concat[0].concatenate_data(
            to_concat[1:],
            axis=self.colVar.get(),
            join="outer" if self.joinVar.get() == True else "inner",
            names=[
                self.data_list.item(key)["text"].split(" (")[0]
                for key in self.data_list.selection()
            ],
        )

        self.data[self.nameVar.get()] = {
            "scdata": scdata,
            "wb": None,
            "state": tk.BooleanVar(),
            "genes": scdata.data.columns.values,
    ...
    def _deleteDataItem(self, event):
        ...
        for key in self.data_list.selection():
            ...
            if name in self.data:
                ...
                data_set_name = self.data_list.item(self.data_list.parent(key))[
                    "text"
                ].split(" (")[0]
    ...
    def _updateSelection(self, event):
        ...
        for key in self.data_list.selection():
            ...
            if "PCA" in name:
                ...
                for col in self.data[data_set][
                    "scdata"
                ].diffusion_eigenvectors.columns.values:
                    self.data_detail.insert("", "end", text=col, open=True)
    ...
    def showRawDataDistributions(self, file_type="csv"):
        ...
        self.dataDistributions.title(
            self.fileNameEntryVar.get() + ": raw data distributions"
        )
        ...
        canvas.get_tk_widget().grid(
            column=0, row=0, rowspan=10, columnspan=4, sticky="NSEW"
        )
        del scdata
    ...
    def runPCA(self):
        ...
        tk.Checkbutton(
            self.pcaOptions, text=u"Randomized PCA (faster)", variable=self.randomVar
        ).grid(column=0, row=2, columnspan=2)

        tk.Button(self.pcaOptions, text="Run", command=self._runPCA).grid(
            column=1, row=4
        )
    ...
    def _runPCA(self):
        for key in self.data_list.selection():
            ...
            self.data[curKey]["scdata"].run_pca(
                n_components=self.nComponents.get(), random=self.randomVar.get()
            )
            ...
            self.data_list.insert(
                ...
                + " x "
                + str(self.data[curKey]["scdata"].pca.shape[1])
                + ")",
                open=True,
            )
        self.visMenu.entryconfig(1, state="normal")
    ...
    def runTSNE(self):
        for key in self.data_list.selection():
            ...
            if self.data[name]["scdata"].data_type == "sc-seq":
                ...
                tk.Entry(self.tsneOptions, textvariable=self.nCompVar).grid(
                    column=1, row=0
                )

            tk.Label(
                self.tsneOptions, text=u"Perplexity:", fg="black", bg="white"
            ).grid(column=0, row=1)
            ...
            tk.Label(
                self.tsneOptions, text=u"Number of iterations:", fg="black", bg="white"
            ).grid(column=0, row=2)
            self.iterVar = tk.IntVar()
            self.iterVar.set(1000)
            tk.Entry(self.tsneOptions, textvariable=self.iterVar).grid(column=1, row=2)

            tk.Label(self.tsneOptions, text=u"Theta:", fg="black", bg="white").grid(
                column=0, row=3
            )
    ...
    def _runTSNE(self):
        ...
        self.data[name]["scdata"].run_tsne(
            n_components=self.nCompVar.get(),
            perplexity=self.perplexityVar.get(),
            n_iter=self.iterVar.get(),
            theta=self.angleVar.get(),
        )
        ...
        self.data_list.insert(
            ...
            + " x "
            + str(self.data[name]["scdata"].tsne.shape[1])
            + ")",
            open=True,
        )
    ...
    def runDM(self):
        for key in self.data_list.selection():
            ...
            tk.Label(
                self.DMOptions, text=u"Number of components:", fg="black", bg="white"
            ).grid(column=0, row=0)
            self.nCompVar = tk.IntVar()
            ...
            tk.Label(
                self.DMOptions,
                text=u"Number of PCA components:",
                fg="black",
                bg="white",
            ).grid(column=0, row=1)
            ...
            tk.Checkbutton(
                self.DMOptions,
                text=u"Randomized PCA (faster)",
                variable=self.randomPCAVar,
            ).grid(column=0, row=2, columnspan=2)

            tk.Label(self.DMOptions, text=u"k:", fg="black", bg="white").grid(
                column=0, row=3
            )
            self.kVar = tk.IntVar()
            self.kVar.set(30)
            ...
            tk.Label(self.DMOptions, text=u"ka:", fg="black", bg="white").grid(
                column=0, row=4
            )
            self.autotuneVar = tk.IntVar()
            self.autotuneVar.set(10)
            tk.Entry(self.DMOptions, textvariable=self.autotuneVar).grid(
                column=1, row=4
            )

            tk.Label(self.DMOptions, text=u"Epsilon:", fg="black", bg="white").grid(
                column=0, row=5
            )
            ...
            tk.Label(
                self.DMOptions,
                text=u"(Epsilon 0 is the uniform kernel)",
                fg="black",
                bg="white",
            ).grid(column=0, columnspan=2, row=6)

            tk.Button(self.DMOptions, text="Run", command=self._runDM).grid(
                column=1, row=7
            )
            tk.Button(
                self.DMOptions, text="Cancel", command=self.DMOptions.destroy
            ).grid(column=0, row=7)
    ...
    def _runDM(self):
        for key in self.data_list.selection():
            ...
            self.data[name]["scdata"].run_diffusion_map(
                n_diffusion_components=self.nCompVar.get(),
                epsilon=self.epsilonVar.get(),
                n_pca_components=self.nPCAVar.get(),
                k=self.kVar.get(),
                ka=self.autotuneVar.get(),
            ...
            self.data_list.insert(
                ...
                + " x "
                + str(self.data[name]["scdata"].diffusion_eigenvectors.shape[1])
                + ")",
                open=True,
            )
    ...
    def runMagic(self):
        for key in self.data_list.selection():
            ...
            self.magicOptions.title(
                self.data_list.item(key)["text"].split(" (")[0] + ": MAGIC options"
            )
            self.curKey = key

            tk.Label(
                self.magicOptions, text=u"# of PCA components:", fg="black", bg="white"
            ).grid(column=0, row=1)
            self.nCompVar = tk.IntVar()
            self.nCompVar.set(20)
            tk.Entry(self.magicOptions, textvariable=self.nCompVar).grid(
                column=1, row=1
            )
            ...
            self.randomVar.set(True)
            tk.Checkbutton(
                self.magicOptions, text=u"Randomized PCA", variable=self.randomVar
            ).grid(column=0, row=2, columnspan=2)

            tk.Label(self.magicOptions, text=u"t:", fg="black", bg="white").grid(
                column=0, row=3
            )
            self.tVar = tk.IntVar()
            self.tVar.set(6)
            ...
            tk.Label(self.magicOptions, text=u"k:", fg="black", bg="white").grid(
                column=0, row=4
            )
            self.kVar = tk.IntVar()
            self.kVar.set(30)
            ...
            tk.Label(self.magicOptions, text=u"ka:", fg="black", bg="white").grid(
                column=0, row=5
            )
            self.autotuneVar = tk.IntVar()
            self.autotuneVar.set(10)
            tk.Entry(self.magicOptions, textvariable=self.autotuneVar).grid(
                column=1, row=5
            )

            tk.Label(self.magicOptions, text=u"Epsilon:", fg="black", bg="white").grid(
                column=0, row=6
            )
            self.epsilonVar = tk.IntVar()
            self.epsilonVar.set(1)
            tk.Entry(self.magicOptions, textvariable=self.epsilonVar).grid(
                column=1, row=6
            )
            ...
            tk.Label(
                ...
                fg="black",
                bg="white",
            ).grid(column=0, columnspan=2, row=7)

            self.rescaleVar = tk.IntVar()
            self.rescaleVar.set(99)
            tk.Label(
                self.magicOptions, text=u"Rescale data to ", fg="black", bg="white"
            ).grid(column=0, row=8)
            tk.Entry(self.magicOptions, textvariable=self.rescaleVar).grid(
                column=1, row=8
            )
            tk.Label(
                self.magicOptions, text=u" percentile", fg="black", bg="white"
            ).grid(column=2, row=8)
            tk.Label(
                self.magicOptions,
                text=u"0 is no rescale (use for log-transformed data).",
            ).grid(row=9, column=0, columnspan=2)

            tk.Button(
                self.magicOptions, text="Cancel", command=self.magicOptions.destroy
            ).grid(column=0, row=10)
            tk.Button(self.magicOptions, text="Run", command=self._runMagic).grid(
                column=1, row=10
            )
    ...
    def _runMagic(self):
        ...
        tk.Label(
            self.magicProgress,
            text="Running MAGIC - refer to console for progress updates.",
        ).grid(column=0, row=0)
        ...
        self.data[name]["scdata"].run_magic(
            ...
            k=self.kVar.get(),
            epsilon=self.epsilonVar.get(),
            rescale_percent=self.rescaleVar.get(),
            ka=self.autotuneVar.get(),
            random_pca=self.randomVar.get(),
        )

        self.data[name + " MAGIC"] = {
            "scdata": self.data[name]["scdata"].magic,
            "wb": None,
            "state": tk.BooleanVar(),
            "genes": self.data[name]["scdata"].magic.data.columns.values,
            "gates": {},
        }
        ...
        self.data_list.insert(
            ...
            + " x "
            + str(self.data[name]["scdata"].magic.data.shape[1])
            + ")",
            open=True,
        )
    ...
    def plotPCAVariance(self):
        for key in self.data_list.selection():
            ...
            self.plotOptions.title(
                self.data_list.item(key)["text"].split(" (")[0]
                + ": % explained PCA plot options"
            )
            self.curKey = key

            tk.Label(
                self.plotOptions, text=u"# of PCA components:", fg="black", bg="white"
            ).grid(column=0, row=1)
            self.nCompVar = tk.IntVar()
            ...
            self.randomVar.set(True)
            tk.Checkbutton(
                self.plotOptions, text=u"Randomized PCA", variable=self.randomVar
            ).grid(column=0, row=2, columnspan=2)

            tk.Button(
                self.plotOptions, text="Cancel", command=self.plotOptions.destroy
            ).grid(column=0, row=3)
            tk.Button(self.plotOptions, text="Run", command=self._plotPCAVariance).grid(
                column=1, row=3
            )
    ...
    def _plotPCAVariance(self):
        ...
        self.fig, self.ax = self.data[name]["scdata"].plot_pca_variance_explained(
            n_components=self.nCompVar.get(), fig=self.fig, random=self.randomVar.get()
        )
        ...
        tk.Button(
            self.tabs[len(self.tabs) - 1][0], text="Save", command=self.savePlot
        ).grid(row=0, column=5, sticky="NE")
        tk.Button(
            self.tabs[len(self.tabs) - 1][0],
            text="Close tab",
            command=self.closeCurrentTab,
        ).grid(row=1, column=5, sticky="NE")
    ...
    def plotPCA_DM(self):
        ...
        self.getScatterSelection(
            plot_type=plot_type,
            options=self.data[name]["scdata"].pca.columns.values
            if plot_type == "PCA"
            else self.data[name]["scdata"].diffusion_eigenvectors.columns.values,
        )
        ...
        if (len(colorSelection) == 1 and len(colorSelection[0]) > 0) or len(
            colorSelection
        ) > 1:
            ...
            for i in range(len(keys)):
                ...
                for j in range(len(colorSelection)):
                    ...
                    if colorSelection[j] in self.data[name][
                        "scdata"
                    ].extended_data.columns.get_level_values(1):
                        self.data[name]["scdata"].scatter_gene_expression(
                            components,
                            fig=self.fig,
            ...
            self.tabs.append([tk.Frame(self.notebook), self.fig])
            self.notebook.add(
                self.tabs[len(self.tabs) - 1][0], text=self.plotNameVar.get()
            )
    ...
    def plotTSNE(self):
        ...
        if (len(self.colorSelection) == 1 and len(self.colorSelection[0]) > 0) or len(
            self.colorSelection
        ) > 1:
            if len(self.colorSelection) == 1 and len(keys) == 1:
                self.fig = plt.figure(
                    figsize=[6 * len(self.colorSelection), 6 * len(keys)]
                )
            else:
                self.fig = plt.figure(
                    figsize=[4 * len(self.colorSelection), 4 * len(keys)]
                )
            ...
            for i in range(len(keys)):
                ...
                for j in range(len(self.colorSelection)):
                    ...
                    if "PC" in self.colorSelection[j]:
                        self.fig, self.ax = self.data[name]["scdata"].plot_tsne(
                            fig=self.fig,
                            ax=self.ax,
                        ...
                        self.fig, self.ax = self.data[name]["scdata"].plot_tsne(
                            fig=self.fig,
                            ax=self.ax,
                            color=self.data[name]["scdata"].diffusion_eigenvectors[
                        ...
                        self.colorSelection[j] in self.data[name]["scdata"].data.columns
                    ):
                        self.fig, self.ax = self.data[name]["scdata"].plot_tsne(
                            fig=self.fig,
                            ax=self.ax,
                        ...
                        self.data[name]["scdata"].plot_tsne(
                            fig=self.fig, ax=self.ax, density=True
                        )
                    else:
                        self.data[name]["scdata"].plot_tsne(
                            fig=self.fig, ax=self.ax, color=self.colorSelection[j]
                        )
    ...
    def scatterPlot(self):
        ...
        if "tSNE" in self.data_list.item(keys[0])["text"]:
            ...
            "PCA" in self.data_list.item(keys[0])["text"]
            or "Diffusion components" in self.data_list.item(keys[0])["text"]
        ):
            self.plotPCA_DM()
        else:
            self.getScatterSelection()
            ...
            if (
                len(xSelection[0]) > 0
                and len(ySelection[0]) > 0
                and len(xSelection) == len(ySelection)
            ):
                ...
                for i in range(len(keys)):
                    ...
                    for j in range(len(xSelection)):
                        ...
                        if len(zSelection[0]) > 0 and len(zSelection) == len(
                            xSelection
                        ):
                            self.ax.append(
                                self.fig.add_subplot(gs[i, j], projection="3d")
                            )
                        ...
                        if colorSelection[j] in self.data[name][
                        ...
                        elif "MAGIC " + colorSelection[j] in self.data[name][
                            "scdata"
                        ].extended_data.columns.get_level_values(1):
                            ...
                            if colorSelection[j] in self.data[name][
                                "scdata"
                            ].magic.extended_data.columns.get_level_values(1):
                                color_ind = np.where(
                                    [
                                        colorSelection[j] in col
                                        for col in self.data[name][
                                            "scdata"
                                        ].magic.extended_data.columns.values
                                    ]
                                )[0][0]
                                color = self.data[name][
                                    "scdata"
                                ].magic.extended_data.columns.values[color_ind]
                                self.data[name]["scdata"].scatter_gene_expression(
                                    genes,
                                    fig=self.fig,
                                    ax=self.ax[len(self.ax) - 1],
                                    color=self.data[name]["scdata"].magic.extended_data[
                                        color
                                    ],
                                )
                ...
                self.canvas = FigureCanvasTkAgg(
                    self.fig, self.tabs[len(self.tabs) - 1][0]
                )
                self.canvas.show()
                self.canvas.get_tk_widget().grid(
                    column=1, row=1, rowspan=10, columnspan=4, sticky="NSEW"
    ...
    def getScatterSelection(self, plot_type="", options=None):
        ...
        tk.Label(
            self.scatterSelection,
            text="For plotting axes, specify a single gene, diffusion component(DC#),"
            + " or PCA component(PC#) or a comma separated list (number of items in"
            + " each list must be equal). The z-axis is optional",
            wraplength=500,
        ).grid(row=0, column=0, rowspan=2, columnspan=2)
        tk.Label(
            self.scatterSelection,
            text="A plot can be colored by a gene, diffusion component(DC#),"
            + ' PCA component(PC#), or "density" for kernel density. '
            + 'The color can also be a solid color(eg: "blue").',
            wraplength=500,
        ).grid(row=2, column=0, rowspan=2, columnspan=2)
        # plot name
        ...
        tk.Entry(self.scatterSelection, textvariable=self.plotNameVar).grid(
            row=4, column=1
        )
        ...
        if plot_type == "tsne":
            tk.Label(
                self.scatterSelection, text=u"x: tSNE1", fg="black", bg="white"
            ).grid(row=5, column=0)
        elif plot_type in ["PCA", "Diffusion components"]:
            tk.Label(self.scatterSelection, text=u"x:", fg="black", bg="white").grid(
                row=5, column=0
            )
            self.xVar = tk.StringVar()
            self.xVar.set(options[0])
            tk.OptionMenu(self.scatterSelection, self.xVar, *options).grid(
                row=5, column=1
            )
        else:
            tk.Label(self.scatterSelection, text=u"x:", fg="black", bg="white").grid(
            ...
            self.xVar = tk.StringVar()
            tk.Entry(self.scatterSelection, textvariable=self.xVar).grid(
                column=1, row=5
            )
            tk.Button(
                self.scatterSelection,
                text="Choose from feature list",
                command=self.getXSelection,
            ).grid(column=2, row=5)

        # y
        if plot_type == "tsne":
            tk.Label(
                self.scatterSelection, text=u"y: tSNE2", fg="black", bg="white"
            ).grid(row=6, column=0)
        elif plot_type in ["PCA", "Diffusion components"]:
            tk.Label(self.scatterSelection, text=u"y:", fg="black", bg="white").grid(
                row=6, column=0
            )
            self.yVar = tk.StringVar()
            self.yVar.set(options[0])
            tk.OptionMenu(self.scatterSelection, self.yVar, *options).grid(
                row=6, column=1
            )
        else:
            tk.Label(self.scatterSelection, text=u"y:", fg="black", bg="white").grid(
            ...
            self.yVar = tk.StringVar()
            tk.Entry(self.scatterSelection, textvariable=self.yVar).grid(
                column=1, row=6
            )
            tk.Button(
                self.scatterSelection,
                text="Choose from feature list",
                command=self.getYSelection,
            ).grid(column=2, row=6)
        ...
        if plot_type != "tsne":
            ...
            if plot_type in ["PCA", "Diffusion components"]:
                tk.Label(
                    self.scatterSelection, text=u"z:", fg="black", bg="white"
                ).grid(row=7, column=0)
                self.zVar.set("None")
                tk.OptionMenu(self.scatterSelection, self.zVar, "None", *options).grid(
                    row=7, column=1
                )
            else:
                tk.Label(
                ...
                tk.Entry(self.scatterSelection, textvariable=self.zVar).grid(
                    column=1, row=7
                )
                tk.Button(
                    self.scatterSelection,
                    text="Choose from feature list",
                    command=self.getZSelection,
                ).grid(column=2, row=7)

        # color
        tk.Label(self.scatterSelection, text=u"color:", fg="black", bg="white").grid(
            row=8, column=0
        )
        self.colorVar = tk.StringVar()
        self.colorVar.set("blue")
        tk.Entry(self.scatterSelection, textvariable=self.colorVar).grid(
            column=1, row=8
        )
        tk.Button(
            self.scatterSelection,
            text="Choose from feature list",
            command=self.getColorSelection,
        ).grid(column=2, row=8)

        tk.Button(
            self.scatterSelection, text="Plot", command=self.scatterSelection.destroy
        ).grid(column=1, row=9)
        tk.Button(
            self.scatterSelection, text="Cancel", command=self._cancelScatter
        ).grid(column=0, row=9)
    ...
    def _getColorSelection(self, event):
        self.colorVar.set(
            self.data_detail.item(self.data_detail.selection()[0])["text"]
        )
    ...
    def savePlot(self):
        ...
        self.plotFileName = filedialog.asksaveasfilename(
            title="Save Plot", defaultextension=".png", initialfile=default_name
        )
